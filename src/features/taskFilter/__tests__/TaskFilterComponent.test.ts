...
describe('TaskFilterComponent', () => {
  ...
  it('should call toggleCompletedVisibility when checkbox changes', async () => {
    ...
    expect(taskFilterStore.toggleCompletedVisibility).toHaveBeenCalledWith(
      false,
    );
  });
  ...
  it('should call togglePriorityFilter when priority button is clicked', async () => {
    ...
    expect(taskFilterStore.togglePriorityFilter).toHaveBeenCalledWith(
      TaskPriority.Low,
    );
  });
  ...
  it('should mark priority buttons as active based on selected priorities', async () => {
    ...
    const allActiveButtons = wrapper.findAll(
      '.task-filter__priority-btn.active',
    );
    ...
    const updatedActiveButtons = wrapper.findAll(
      '.task-filter__priority-btn.active',
    );
