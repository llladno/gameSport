...
export const useTaskFilter = defineStore('taskFilterStore', () => {
  ...
  const togglePriorityFilter = (priority: TaskPriority): void => {
    if (filterOptions.value.priorities.includes(priority)) {
      filterOptions.value.priorities = filterOptions.value.priorities.filter(
        (p) => p !== priority,
      );
    } else {
  ...
  const applyFilters = (tasks: Task[]): Task[] => {
    return tasks.filter((task) => {
      ...
      const matchesCompletedFilter =
        filterOptions.value.showCompleted || !task.completed;

      const matchesPriorityFilter = filterOptions.value.priorities.includes(
        task.priority,
      );
  ...
  const getFilterOptions = computed<TaskFilterOptions>(
    () => filterOptions.value,
  );
